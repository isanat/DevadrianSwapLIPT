...
>(
  (
    ...
    const [open, setOpen] = useLocalStorage<boolean>(SIDEBAR_COOKIE_NAME, true)
  ...
  (
    ...
    const button = (
      <Comp
        ...
        className={cn(
          buttonVariants({ size: "icon", className: "size-auto p-2" }),
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn("h-8 flex gap-2 px-2 items-center rounded-md", className)}
      {...props}
    >
      {showIcon && (
        <Skeleton
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  const pathname = usePathname()
  const active = isActive ?? (props.href ? pathname === props.href : false)
