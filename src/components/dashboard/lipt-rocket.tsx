...
export function LiptRocket() {
  ...
  const startGame = useCallback(() => {
    ...
    // Limpa fumaça
    rocket.smoke.forEach(p => p.destroy());
    rocket.smoke = [];
    ...
    const animate = () => {
      if(gameStatus === 'cashed_out' || !appRef.current || !rocketRef.current) {
        if(animationFrameId.current) cancelAnimationFrame(animationFrameId.current);
        return;
      }
      ...
      for (let i = 0; i < emitRate; i++) {
        ...
        p.circle(0, 0, Math.random() * 3 + 2).fill({ color: 0xdddddd, alpha: 0.7 + Math.random() * 0.2 });
      ...
      for (let i = smoke.length - 1; i >= 0; i--) {
        const p = smoke[i] as PIXI.Graphics & { vx: number, vy: number, life: number };
        p.x += p.vx;
        p.y += p.vy;
        p.vy += 0.06;
        p.alpha -= 0.02;
  ...
  const handleReset = () => {
    ...
    if (rocketRef.current && appRef.current) {
      ...
      rocketRef.current.x = appRef.current.screen.width / 2;
